...
continua a identificare la posizione fisica del documento, ma non viene
consegnato un token fisico.

QR code e barcode sono equivalenti dal punto di vista applicativo: entrambi
decodificano il valore del `copy_identifier`. Non esistono quindi modalità
separate `qr` e `barcode`.
...
Possono essere preparati, importati o modificati anche mentre l'Event usa la
modalità `token`.
...
| `identification_mode` (`token` / `copy_identifier`) | `game_library` dell'Event |
